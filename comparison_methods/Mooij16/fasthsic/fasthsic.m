function [p, HSIC, prob, logp] = fasthsic(X,Y,sX,sY,nrperm)
% function [p, HSIC, prob, logp] = fasthsic(X,Y,sX,sY,nrperm)
% 
...
%           prob    = probability density of the HSIC (not implemented in this MatLab fallback)
%           logp    = log p-value of the HSIC
%
% Copyright (c) 2013  Joris Mooij  <j.m.mooij@uva.nl>"
% All rights reserved.  See the file LICENSE for the license terms"
  ...
  % probability density of HSIC: not implemented here
  prob = nan;

  if p == 0.0 % Underflow occurred. Deal with this using the solution proposed on http://math.stackexchange.com/questions/1041674/log-of-1-reguralized-incomplete-gamma-function-upper 
    x = N * HSIC / b;
    logp = -x + a * log(x) - gammaln(a) + log(1.0 / (x + (1.0 - a) / (1.0 + 1.0 / (x + (2.0 - a) / (1.0 + 2.0 / x)))));
  else
    logp = log(p);
  end
