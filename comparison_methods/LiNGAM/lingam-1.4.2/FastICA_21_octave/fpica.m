...
% Antti Kerminen 11.7.2006: In symmetric mode, returns W and A even
% if the fixed point loop did not converge.
...
if approachMode == 1,
  ...
  for round = 1:maxNumIterations + 1,
    if round == maxNumIterations + 1,
      ...
      % Return W and A if possible. - Antti Kerminen 11.7.2006
      if ~isempty(B)
	  W = B' * whiteningMatrix;
