...
power_spectrum_beta = lambda q: 512/((q/1)**4 + 1)
power_spectrum_f = lambda q: 512/((q/1)**4 + 1)
...
for i in range(100):
    ...
    if flip:
        np.savetxt(
                './benchmarks/bcs_nvar5e-2/pair0{:03d}.txt'.format(i+1),
                np.array([y, x]).T, delimiter=' ')

        with open('./benchmarks/bcs_nvar5e-2/pairmeta.txt', 'a') as f:
