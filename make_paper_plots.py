...
plt.rc('text', usetex=True)
plt.rc('text.latex',
        preamble=r'\usepackage{amsmath} \usepackage{bm} \usepackage{dsfont}')
ax.set_xlabel('$X$')
ax.set_ylabel('$Y$')
...
ax.plot(beta0_x_to_y, label=r'$\beta_{min}$', c='black')
ax.set_xlabel('$X$')
...
bar_width = 0.35
rects1 = ax.bar(index, term_values_x_to_y + [evidence_x_to_y,], bar_width,
                fill=False,
                hatch='///',
                label=r'$X \rightarrow Y$')
rects2 = ax.bar(index + bar_width, term_values_y_to_x + [evidence_y_to_x],
                bar_width,
                fill=False,
                hatch='///\\\\\\',
